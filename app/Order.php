...
{
    protected $guarded = [];
    protected $appends = ['name', 'admin_revenue','ambassador_revenue'];
    ...
    {
        return $this->revenue('admin_revenue');
    }

    public function getAmbassadorRevenueAttribute()
    {
        usleep(100000);
        return $this->revenue('ambassador_revenue');
    }

    function revenue($scope){
        return $this->orderItems->sum(function(OrderItem $item)use($scope){
            return $item->$scope;
        });
    }
}
